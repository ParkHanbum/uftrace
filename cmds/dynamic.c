...
extern void check_perf_event(struct opts *opts);
...
char *get_libmcount_dynamic_path(struct opts *opts)
{
	char *lib = xmalloc(PATH_MAX);
	char *libname = "libmcount-dynamic.so";

	if (opts->lib_path) {
		snprintf(lib, PATH_MAX, "%s/libmcount/%s", opts->lib_path, libname);

		if (access(lib, F_OK) == 0) {
			return lib;
		}
		else if (errno == ENOENT) {
			snprintf(lib, PATH_MAX, "%s/%s", opts->lib_path, libname);
			if (access(lib, F_OK) == 0)
				return lib;
		}
		free(lib);
...
#ifdef INSTALL_LIB_PATH
	snprintf(lib, PATH_MAX, "%s/%s", INSTALL_LIB_PATH, libname);
...
void do_inject(int pfd[2], int ready, struct opts *opts, char *argv[])
{
	...
	close(pfd[0]);
	make_uftrace_environ_file(opts, pfd[1]);

	if (read(ready, &dummy, sizeof(dummy)) != (ssize_t)sizeof(dummy))
		pr_err("waiting for parent failed");

	libmcount_path = get_libmcount_dynamic_path(opts);

	if (libmcount_path == NULL)
		pr_err("connot found shared object at %s", libmcount_path);

	inject(libmcount_path, target_pid);
	sleep(2);
	abort();
}
...
static void check_binary_dynamic_avilable(struct opts *opts)
{
	...
	if (chk) {
		if (chk < 0)
			pr_err_ns("Cannot check '%s'", opts->exename);
		else
