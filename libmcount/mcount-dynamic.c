...
#define SETROX(addr, len)   mprotect((void*)((addr) &~ 0xFFF),\
                                     (len) + ((addr) - ((addr) &~ 0xFFF)),\
                                     PROT_READ | PROT_EXEC)
#define SETOOX(addr, len)   mprotect((void*)((addr) &~ 0xFFF),\
                                     (len) + ((addr) - ((addr) &~ 0xFFF)),\
                                     PROT_EXEC)
...
extern int mcount_entry(unsigned long *parent_loc, unsigned long child,
...
puchar patch_code(uintptr_t addr, unsigned char* call_insn, unsigned int code_size) 
{
	...
	uintptr_t alloc_addr = (uintptr_t)malloc(saved_code_size);	
	SETRWX(alloc_addr, saved_code_size);
	memset((void *)alloc_addr, 0, saved_code_size); 
	...
	*((uintptr_t *)(&save_addr[code_size+6])) = (uintptr_t)&code_addr[code_size];

	// remove permissions exclude executable to keep the code safety. 
	// SETROX(alloc_addr, code_size+6+8);

	pr_dbg2("RETURN ADDRESS : %llx\n", &code_addr[code_size]);
	
	if (debug) {
		print_disassemble((uintptr_t)save_addr, saved_code_size); 
		print_disassemble((uintptr_t)addr, code_size);
	}
...
*/
void dynamic_instrument(uintptr_t address, uint32_t insn_size) 
{
	struct address_entry* el;
	puchar saved_code;
	saved_code = patch_code(address, (puchar)g_call_insn, insn_size);
	if (saved_code) {
		...
		el->addr = address;
		el->saved_addr = (uintptr_t)saved_code;
...
uintptr_t find_origin_code_addr(uintptr_t addr)
{
	...
	list_for_each_entry(entry, &address_list, list) {
		...
		if (entry->addr == patched_addr) {
			pr_dbg("%lx matched -  %lx\n", patched_addr, entry->addr);
		 ...
		 struct mcount_regs *regs)
{
	...
	print_disassemble(child-0x6, 0x6);
	int result;
	result = mcount_entry(parent_loc, child, regs);

	if (!result) {
		pr_dbg2("Let's find original code :  %d\n", result);
		...
		// temporary, 
		// sometime mcount_entry return the -1 for unknown reason. 
		/*
		pr_dbg2("MCOUNT_ENTRY RETURN  : %d\n", result);
		uintptr_t origin_code_addr = find_origin_code_addr(child);
		return origin_code_addr;
		*/
	}
...
void remove_write_perm_to_text()
{
	struct uftrace_mmap *map, *curr;
	map = symtabs.maps;
	pr_dbg("CHECK MMAP \n");
...
void enable_dynamic_trace()
{
	...
	// remove write permission from each text secion. 
	// remove_write_perm_to_text();
	
	// prints "original code address : saved address" pair.
	if (debug) {
		pr_dbg("========= [INSTRUMENTED FUNCTIONS] ==========\n");
		list_for_each_entry(entry, &address_list, list) {
			struct symtab uftrace_symtab = symtabs.symtab;
			// do dynamic instrumentation to each function.
			for(index=0;index < uftrace_symtab.nr_sym;index++) {
