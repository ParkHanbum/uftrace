...
int inject(char *libname, pid_t pid)
{
	...
	if (!lib_path)
		pr_err_ns("can't find file %s", libname);
	...
	if (target_libdl_addr == 0)
		pr_err_ns("Libdl does not loaded from target process");
	...
	if (lib_addr == 0) {
		...
		free(backup);
		free(injected_area);
		pr_err_ns("dlopen() failed to load %s", libname);
	}
	...
	if (check_loaded(target, libname))
		...
		pr_err_ns("could not inject '%s'", libname);
